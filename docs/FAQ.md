...
### Почему статус подключения зелёный, но в Нодах пусто?

**Причины:**
1. Запрос конфигурации (`want_config_id`) не дошёл до устройства
2. Прошивка временно подвисла после загрузки

**Решение:**
1. На экране «Ноды» **тапните по статус-баннеру вверху** — это переотправит `want_config_id` и устройство снова пришлёт NodeInfo
2. Если не помогло — отключитесь на экране «Соединение» и подключитесь снова
3. Проверьте логи: `adb logcat | Select-String "BleManager"` — там должны быть строки про `onCharacteristicRead FromRadio`

### Почему в Соединении не находит устройство?

**Причины:**
1. Устройство выключено или вне радиуса 5 м
2. Не выданы разрешения BLUETOOTH_SCAN / ACCESS_FINE_LOCATION
3. Bluetooth выключен в системе

**Решение:**
1. Откройте «Соединение» → проверьте чип внизу: «Нет соединения» → нажмите «Сканирование»
2. Если разрешения не выданы — нажмите ещё раз, появится системный диалог
3. На Android 12+ нужны именно `BLUETOOTH_SCAN` + `BLUETOOTH_CONNECT`, на старых — `ACCESS_FINE_LOCATION`

### Что делать с табами «Сеть» и «COM-порт»?

Они показывают Toast «не реализовано». В текущей версии работает только Bluetooth. Поддержка TCP (Wi-Fi к устройству) и USB OTG — в направлениях развития.
...
> ⚠️ В версии 0.2.0 apply-на-устройство работает **только для имени канала и PSK primary-канала**. Остальные 9 разделов настроек сохраняются только локально.

---

## E2E личные сообщения

### Что нужно чтобы написать кому-то в личку?

Две вещи **от собеседника**:
1. Его **node-ID** (`!aabbccdd`) — видно в карточке его узла на экране «Ноды» в правом нижнем углу с иконкой отпечатка пальца
2. Его **публичный E2E-ключ** (длинная hex-строка ~178 символов) — он копирует у себя через **Настройки → Безопасность → Копировать**

После этого: вкладка **«E2E ЛС»** → ввести ID + ключ → написать → «Отправить E2E».

### Как передать собеседнику свой публичный ключ?

Любым способом:
- Через общий чат: скопируй свой pubkey из Настроек → отправь как обычное сообщение. Безопасно (ключ публичный), но все в канале увидят факт переписки.
- Внешним каналом: WhatsApp, Telegram, при встрече.

⚠️ Автоматический обмен ключами через служебное сообщение `E2E-KEY:` — направление развития, пока вручную.

### Где хранится мой приватный ключ?

В **Android Keystore** — защищённой памяти, к которой даже сама ОС обычно не имеет доступа. Конкретно: PKCS#8 приватный ключ зашифрован через AES-256-GCM, а AES-ключ создан и хранится в Keystore с alias `e2e_wrap_key_v1`.

При переустановке приложения или сбросе данных ключ пересоздаётся — старые сообщения уже не расшифровать.

### Что значит «[Не удалось расшифровать]»?

Получатель использует приватный ключ, который не соответствует тому публичному ключу, которым ты зашифровал. Причины:
- Он переустановил приложение → ключ изменился
- Ты вставил битый/неполный hex-ключ
- Это другое устройство того же пользователя

**Решение:** запросить у него свежий публичный ключ.

### Что значит ✓✓ под исходящим сообщением?

- **⏱** — сообщение отправляется
- **✓** — записано в радио (прошивка приняла ToRadio)
- **✓✓** — доставлено получателю (получен Routing ACK) *— в текущей версии всегда показывается **✓**, обработка ACK в направлениях развития*
- **⚠** — ошибка отправки или таймаут 30 секунд
